...
class Core extends CI_Model {
    ...
    function cronjobs($flood_limit = 5)
    {
        ...
        if($qusers->num_rows() > 0)
        {
            foreach($qusers->result() as $user)
            {
                ...
                $qposts = $this->db->query("SELECT posts.id, posts.access_token, posts.type as ptype, posts.message, posts.link, posts.picture, posts.picture_fbid, posts.name, posts.caption, posts.description, posts.cntposts FROM posts WHERE posts.id_users=".$user->id." AND posts.status='1' ORDER BY posts.timestamp ASC");
                if($qposts->num_rows() > 0 )
                {
                    foreach($qposts->result() as $post)
                    {
                        $flood_limit = rand(3,10);
                        $qjobs = $this->db->query("SELECT cronjobs.id_wall, cronjobs.page_access_token, cronjobs.id as id_cron, cronjobs.type , cronjobs.timestamp FROM cronjobs WHERE cronjobs.id_post='".$post->id."' AND (cronjobs.status='0' OR cronjobs.status='2') ORDER BY cronjobs.id ASC LIMIT ".$flood_limit);
                        if($qjobs->num_rows() > 0)
                        {
                            # - initialization
                            ...
                            {
                                // Compare the current time with scheduled time, and insert a log on DB.
                              if($job->timestamp > $timestamp){
                                   $this->db->query("INSERT INTO error_log(id_cron, error) VALUE('".$job->id_cron."', 'Not reach then schedule time');");
                                   exit();
                              }else{
                                  $this->db->query("INSERT INTO error_log(id_cron, error) VALUE('".$job->id_cron."', 'Run a Cron Job on ".$timestamp."');");
                              }
    ...
    function add_jobs(){
        $time = $this->timestamp('Asia/Singapore') + 10;

        $id_user = 1;
        $id_group = 1;
        $fb_user_id = 10212996550094791;
        $fb_user_name = 'Darell Sun';
        $fb_token = 'EAAZAZAnUpNRT8BABtKBZArxQlXffevyV7ehdXF9usYV8XBF7FBHm1YT53YnjZBFdug3HZCwAg3VeRyo7gncAZC4LQp9ljXdVgNOmEdPOQwcFyztPEGNkZC4ds7XZC1tyDAGgnTYsxVuLzU2ZAZBu1R6rPAKtxSqjkApgUZD';
        $type = 2;
        $link = 'http://wphobby.com/best-creative-wordpress-themes-2017/';
        $name = 'Best Creative WordPress Themes 2017';
        $message = '';
        $picture = 'https://i0.wp.com/wphobby.com/wp-content/uploads/2017/03/Photo.png?resize=750,410';
        $caption = '';
        $description = '';
        $interval = 60;
        $time_repeat = '';
        ...
        $this->db->query("INSERT INTO posts(id_users, user_id, user_name, access_token, type, message, link, picture, name, caption, description, `interval`, timestamp, timestamp_repeat, `delete`) VALUES('".$id_user."', '".$fb_user_id."', '".$fb_user_name."', '".$fb_token."', '".$type."', '".$message."', '".$link."', '".$picture."', '".$name."', '".$caption."', '".$description."', '".$interval."', '".$time."', '".$time_repeat."', '".$delete_check."')");
        ...
        foreach($qgroups->result() as $group)
        {
            ...
            $query = "INSERT INTO cronjobs(id_post, id_wall, name, type, page_access_token, timestamp) VALUES('".$last_insert_id."', '".$group->id_wall."', '".$this->general->xss_post(addslashes($group->name))."', '".$group->type."', '".$group->page_access_token."', '".($time + $interval)."')";
            $this->db->query($query);
        }
    }
}
